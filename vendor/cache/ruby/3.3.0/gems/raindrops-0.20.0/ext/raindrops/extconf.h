...
#define CFUNC_tcp_info_tcpi_snd_wnd static VALUE tcp_info_tcpi_snd_wnd(VALUE self){\
	struct tcp_info *info = DATA_PTR(self);\
	return UINT2NUM((uint32_t)info->tcpi_snd_wnd);}
#define DEFINE_METHOD_tcp_info_tcpi_snd_wnd rb_define_method(cTCP_Info,"snd_wnd",tcp_info_tcpi_snd_wnd,0)
...
#define CFUNC_tcp_info_tcpi_rcv_ooopack static VALUE tcp_info_tcpi_rcv_ooopack(VALUE self){\
	struct tcp_info *info = DATA_PTR(self);\
	return UINT2NUM((uint32_t)info->tcpi_rcv_ooopack);}
#define DEFINE_METHOD_tcp_info_tcpi_rcv_ooopack rb_define_method(cTCP_Info,"rcv_ooopack",tcp_info_tcpi_rcv_ooopack,0)
