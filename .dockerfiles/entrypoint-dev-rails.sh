...
until psql "postgres://${PGUSER}:${PGPASSWORD}@${PGHOST}:5432" -lqt &>/dev/null; do
  echo "waiting for database to start up"
  sleep 5
done
psql "postgres://${PGUSER}:${PGPASSWORD}@${PGHOST}:5432" -lqt 2> /dev/null | cut -d \| -f 1 | grep -wq "markus_${RAILS_ENV}" || rails db:setup
