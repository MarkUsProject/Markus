...
class GradeEntryFormTest < ActiveSupport::TestCase
  ...
  context 'A grade entry form object: ' do
    setup do
      ...
      GradeEntryItem.make!(:grade_entry_form => @grade_entry_form, :out_of => 25, :position => 1)
      GradeEntryItem.make!(:grade_entry_form => @grade_entry_form, :out_of => 50, :position => 2)
      GradeEntryItem.make!(:grade_entry_form => @grade_entry_form, :out_of => 10.5, :position => 3)
    end
  ...
  context 'Calculate the total mark for a student: ' do
    ...
    should 'verify the correct value is returned when the student has grades for none of the questions' do
      grade_entry_student_with_no_grades = GradeEntryStudent.make!(:grade_entry_form => @grade_entry_form)
