...
class GradeEntryFormTest < ActiveSupport::TestCase
  ...
  def test_validate_valid_date
    g = GradeEntryForm.new(:short_identifier => 'T1', :date => 1.day.from_now, is_hidden => false)
    assert g.valid?
  end

  # Make sure validate works appropriately when the date is invalid
  def test_validate_invalid_date
    g = GradeEntryForm.new(:short_identifier => 'T1', :date => '2009-', is_hidden => false)
    assert !g.valid?
  end

  # Make sure that validate allows dates to be set in the past
  def test_validate_date_in_the_past
    g = GradeEntryForm.new(:short_identifier => 'T1', :date => 1.day.ago, is_hidden => false)
    assert g.valid?
  end
