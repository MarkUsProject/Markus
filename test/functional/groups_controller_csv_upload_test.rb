...
class GroupsControllerCsvUploadTest < AuthenticatedControllerTest
  context "An authenticated and authorized admin" do
    ...
    setup do
      ...
      @repository_storage = File.join(RAILS_ROOT, "tmp", "groups_controller_repos")
    ...
    context "on an assignment with allow_web_submits set to false and group max of 1" do
      ...
      setup do
        ...
        @autogen_repos_csv_file = ActionController::TestUploadedFile.new(
                                 File.join( csv_dir, "autogenerated_repos_groups.csv"), "text/plain")
        @non_admin_repos_csv_file = ActionController::TestUploadedFile.new(
                                 File.join( csv_dir, "non_admin_repos_groups.csv"), "text/plain")
        @username_repos_csv_file = ActionController::TestUploadedFile.new(
                                 File.join( csv_dir, "username_repos_groups.csv"), "text/plain")
      ...
      context "should be able to upload groups using CSV file upload, and repos" do
        setup do
          ...
          @res = post_as @admin, :csv_upload, { :id => @assignment.id, :group => { :grouplist=> @username_repos_csv_file } }
        end
      ...
      context "should be able to upload groups using CSV file upload, and repos" do
        ...
        setup do
          ...
          @assignment = Assignment.make(:allow_web_submits => false,
                                        :group_max => 1, :group_min => 1)
          @res = post_as @admin, :csv_upload, { :id => @assignment.id, :group => { :grouplist=> @autogen_repos_csv_file } }
        end
      ...
      context "should be able to upload groups using CSV file upload, and repos" do
        ...
        setup do
          ...
          @res = post_as @admin, :csv_upload, { :id => @assignment.id, :group => { :grouplist=> @non_admin_repos_csv_file } }
        end
