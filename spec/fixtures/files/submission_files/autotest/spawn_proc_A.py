...
if proc.poll() is None:
  print(json.dumps({'name': 'spawned_proc_test_A', 'input': 'NA', 'expected': 'NA', 'actual': 'child process successfully spawned', 'marks_earned': 2, 'marks_total': 2, 'status': 'pass'}))
else:
  print(json.dumps({'name': 'spawned_proc_test_A', 'input': 'NA', 'expected': 'NA', 'actual': 'failed to spawn child process', 'marks_earned': 0, 'marks_total': 2, 'status': 'error'}))
