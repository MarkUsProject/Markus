describe AnnotationCategoriesController do
  ...
  context 'csv_upload' do
    ...
    # the '.read' method is called to simulate the behaviour of the http uploaded file
    ...
    it 'does not accept files with invalid columns' do
      ...
      allow(@file_invalid_column).to receive(:read).and_return(
        File.read(fixture_file_upload('files/annotation_categories/form_invalid_column.csv', 'text/csv')))
    ...
    it 'does not accept fileless submission' do
      ...
      expect(response.status).to eq(302)
      expect(flash[:error].size).to eq(1)
      # print(flash[:error].first) => <p>You must select a file to upload.</p>
      expect(response).to redirect_to(action: 'index', id: assignment.id)
      expect(response.location).to eq('http://test.host/en/assignments/1/annotation_categories?id=1')
    end
    ...
    it 'does not accept a non-CSV file with .CSV extension' do
      ...
      # print (flash[:error].first) =>
      # <p>The selected file is not a CSV file (even though it may have a .csv extension).</p>
      expect(response).to redirect_to(action: 'index', id: assignment.id)
      expect(response.location).to eq('http://test.host/en/assignments/1/annotation_categories?id=1')
    end
    ...
    it 'does not accept any file without .CSV extension' do
      ...
      allow(@yml_file_not_for_csv_upload).to receive(:read).and_return(
        File.read(
          fixture_file_upload('files/annotation_categories/yml_file_should_not_be_saved_by_csv_file.yml', 'text/yml')
        )
      )
      post :csv_upload, params: { assignment_id: assignment.id,
  ...
  context 'yml_upload' do
    it 'accept a valid file' do
      ...
      allow(@valid_yml_file).to receive(:read).and_return(
        File.read(fixture_file_upload('spec/fixtures/files/annotation_categories/valid_yml.yml'))
      )
    ...
    it 'does not accept files with empty annotation category name' do
      ...
      allow(@yml_with_invalid_category).to receive(:read).and_return(
        File.read(fixture_file_upload('files/annotation_categories/yml_with_invalid_category.yml'))
      )
