describe SectionsController do
  ...
  context 'A logged in Admin' do
    ...
    it 'on create new section' do
      post_as @admin, :create, params: { course_id: course.id, section: { name: 'section_01' } }
    ...
    it 'not be able to create a section with the same name as a existing one' do
      post_as @admin, :create, params: { course_id: course.id, section: { name: section.name } }
      expect(response.status).to eq(200)
    ...
    it 'not be able to create a section with a blank name' do
      post_as @admin, :create, params: { course_id: course.id, section: { name: '' } }
    ...
    it 'on edit section' do
      post_as @admin, :edit, params: { course_id: course.id, id: section.id }
      expect(response.status).to eq(200)
    end

    it 'be able to update a section name to "no section"' do
      put_as @admin, :update, params: { course_id: course.id, id: section.id, section: { name: 'no section' } }
    ...
    it 'not see a table if no students in this section' do
      get_as @admin, :edit, params: { course_id: course.id, id: section.id }
      expect(response.body.to_s.match('section_students')).to be_nil
    end

    it 'not be able to edit a section name to an existing name' do
      put_as @admin, :update, params: { course_id: course.id, id: section.id, section: { name: section2.name } }
      expect(response.status).to eq(200)
    ...
    context 'with an already created section' do
      it 'be able to delete a section' do
        delete_as @admin, :destroy, params: { course_id: course.id, id: section.id }
