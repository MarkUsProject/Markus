---
permalink: /instructors/importing-and-exporting-data/
...
Instructors may download a summary of the grades for each assignment in CSV format. They may also upload a CSV from the assignment's Grades tab to update criterion marks. See [Uploading Grades by CSV](assignments/grades.md#uploading-grades-by-csv) for upload instructions.

The CSV file consists of two *header rows* at the top of the file:

- The first row contains the Group name, User Name, Last Name, First Name, Section, ID Number, Email, Total Mark, and each criterion. The last entry is Bonus/Deductions.
- The second row contains dummy entries (it is ignored by MarkUs), followed by the "Out of" total for each column.

Every subsequent row is a student record:

- The first seven entries in the row are the Group name, User Name, Last Name, First Name, Section, ID Number and Email of the student. If any of these fields are not provided, they are left blank.
- Every subsequent entry is the grade of the student for that column. This entry can be left blank if no grade is given.
- The students are ordered by their group. This means all students of the same group will appear consecutively.
...
```csv
Group name,User name,Last name,First name,Section,Id number,Email,Total Mark,dolores,iusto,Bonus/Deductions
, , , , , , ,Out of,10,4,
...
- [properties](importing-and-exporting-data.md#assignments)
- [tags](importing-and-exporting-data.md#tags)
- [criteria](importing-and-exporting-data.md#criteria)
- [annotation categories](importing-and-exporting-data.md#annotation-categories) (excluding one time annotations)
