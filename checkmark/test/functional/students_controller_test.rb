...
class StudentsControllerTest < ActionController::TestCase
  ...
  def name_special_cases
    user = @controller.add_student("g5d'indy,831238216,d'Indy,Vincent")
    assert_not_nil user, "Apostrophe in names"
    assert_equal "831238216", User.find_by_last_name("d'Indy").user_number
    
    user = @controller.add_student("c5zemlin,404258308,Zemlinsky,Alexander von")
  ...
  def csv_format
    user = @controller.add_student("c6malipi,637257456,Malipiero,Gian Francesco,unneeded field")
