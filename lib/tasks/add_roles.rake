namespace :markus do
  task :add_roles, [:roles_file] => :environment do |_task, args|
    ...
    MarkusCsv.parse(File.read(args[:roles_file])) do |row|
      ...
      end_user = EndUser.find_or_create_by!(user_name: data[:user_name]) do |end_user|
      ...
      course.roles.find_or_create_by!(end_user: end_user, type: data[:role_type].capitalize) do |role|
