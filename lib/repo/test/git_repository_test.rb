...
class GitRepositoryTest < Test::Unit::TestCase
  ...
  context "A GitRepository instance" do
    ...
    setup do
      ...
      conf_admin["REPOSITORY_PERMISSION_FILE"] = GIT_AUTHZ_FILE
    ...
    should "be able to retrieve a revision given a valid revision as integer number" do
      ...
      assert_instance_of(Repository::GitRevision, new_revision, "Revision not of class GitRevision")
  ...
  context "A repository with an authorization file specified" do
    ...
    GIT_AUTHZ_FILE = GIT_TEST_REPOS_DIR + "/Git_authz"
    ...
    setup do
      ...
      FileUtils.remove_dir(TEST_REPO, true)
      FileUtils.rm(GIT_AUTHZ_FILE, :force => true)
      # have a clean authz file
      FileUtils.cp(GIT_AUTHZ_FILE + '.orig', GIT_AUTHZ_FILE)
      # create repository first
      ...
      conf_non_admin["REPOSITORY_PERMISSION_FILE"] = GIT_AUTHZ_FILE
  ...
  context "Looping over several repositories" do
    ...
    should "add a user per each repository" do
      # use a different Git_authz file for this test
      old_Git_authz = GIT_AUTHZ_FILE
      new_Git_authz = GIT_TEST_REPOS_DIR + "/Git_authz_bulk_stuff"

      # remove authz file if it exists
      if File.exist?(new_Git_authz)
        FileUtils.rm(new_Git_authz)
      end
  ...
  context "Setting and deleting bulk permissions" do
    setup do
      # use a different Git_authz file for this test
      new_Git_authz = GIT_TEST_REPOS_DIR + "/Git_authz_bulk_stuff2"
      @conf_admin = Hash.new
      @conf_admin["IS_REPOSITORY_ADMIN"] = true
      @conf_admin["REPOSITORY_PERMISSION_FILE"] = new_Git_authz
...
# Test suite for testing proper functioning of
# GitRevision, an implementation of AbstractRevision

#class GitRevisionTest < Test::Unit::TestCase
  # TODO: Test GitRevision here
#end
