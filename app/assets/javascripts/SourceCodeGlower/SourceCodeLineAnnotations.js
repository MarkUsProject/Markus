...
function SourceCodeLineAnnotations(
  line_manager,
...
SourceCodeLineAnnotations.prototype.remove_annotation = function (
  annotation_id,
  range,
  annotation_text_id
) {
...
SourceCodeLineAnnotations.prototype.addRelationship = function (
  annotation_id,
  line_num,
  annotation_text_id
) {
...
SourceCodeLineAnnotations.prototype.relationshipExists = function (
  annotation_id,
  line_num,
  annotation_text_id
) {
  ...
  for (var i = 0; i < relationships.length; i++) {
    ...
    if (
      relationship.line_num == line_num &&
...
SourceCodeLineAnnotations.prototype.updateRelationships = function (
  annotation_id,
  annotation_text_id
) {
  var relationships = this.getRelationships();
...
SourceCodeLineAnnotations.prototype.removeRelationship = function (
  annotation_id,
  line_num,
  annotation_text_id
) {
...
SourceCodeLineAnnotations.prototype.getAnnotationTextsForLineNum = function (
  ...
  for (var i = 0; i < relationships.length; i++) {
    var relationship = relationships[i];
    if (
      relationship.line_num == lineNum &&
      annotationIds.indexOf(relationship.annotation_id.toString()) >= 0
    ) {
      result.push(
        this.getAnnotationTextManager().getAnnotationText(relationship.annotation_text_id)
      );
    }
  }
...
SourceCodeLineAnnotations.prototype.displayTextsForLine = function (lineNum, event, x, y) {
