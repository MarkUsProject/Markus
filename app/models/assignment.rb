...
class Assignment < ActiveRecord::Base
  ...
  def clone_groupings_from(assignment_id)
    ...
    self.transaction do
      ...
      self.group_name_autogenerated =
        original_assignment.group_name_autogenerated
      ...
      original_assignment.groupings.each do |g|
        unhidden_student_memberships =
          g.accepted_student_memberships.select do |m|
          !m.user.hidden
        end
        ...
        unless unhidden_student_memberships.empty?
          ...
          all_memberships =
            unhidden_student_memberships + unhidden_ta_memberships
          all_memberships.each do |m|
            ...
            raise 'Could not save membership' if !(
              grouping.memberships << membership
            )
          end
  ...
  def add_csv_group(row)
    ...
    unless group.errors[:base].blank?
      m_logger.log(
        "Collision detected of '#{group.errors[:base]}'",
        MarkusLogger::ERROR
      )
      collision_error = I18n.t(
        'csv.repo_collision_warning',
  ...
  def get_simple_csv_report
    ...
    CSV.generate do |csv|
      students.each do |student|
        ...
        grouping = student.accepted_grouping_for(id)
  ...
  def grade_distribution_as_percentage(intervals=20)
    ...
    groupings =
      self.groupings.all(include: [{ current_submission_used: :results }])
