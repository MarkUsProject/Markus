...
class Assignment < Assessment
  ...
  def assignment_properties_config
    properties = {}
    other_properties = {}
    fields = [:short_identifier, :description, :due_date, :message, :is_hidden, :show_total]
    fields.each { |f| properties[f] = self.send(f) }
    if self.scanned_exam
      scanned_fields = [:remark_due_date, :remark_message, :assign_graders_to_criteria, :allow_remarks,
                        :display_grader_names_to_students, :display_median_to_students, :has_peer_review,
                        :section_groups_only, :section_due_dates_type, :scanned_exam, :hide_unassigned_criteria]
      scanned_fields.each { |f| other_properties[f] = self.send(f) }
      properties.merge('assignment_properties_attributes': other_properties)
    elsif self.is_peer_review?
      peer_fields = [:group_min, :group_max, :student_form_groups, :assign_graders_to_criteria,
                     :group_name_autogenerated, :group_name_displayed, :invalid_override, :section_groups_only,
                     :section_due_dates_type, :anonymize_groups, :hide_unassigned_criteria]
      peer_fields.each { |f| other_properties[f] = self.send(f) }
      properties.merge('assignment_properties_attributes': other_properties)
    else
      assignment_fields = [:group_min, :group_max, :allow_web_submits, :student_form_groups, :remark_due_date,
                           :remark_message, :assign_graders_to_criteria, :allow_remarks,
                           ...
                           :section_groups_only, :section_due_dates_type, :only_required_files, :anonymize_groups,
                           :hide_unassigned_criteria, :duration, :start_time, :is_timed]
      assignment_fields.each do |f|
        other_properties[f] = self.send(f) if self.is_timed || [:is_timed, :start_time, :duration].exclude?(f)
      end
  ...
  def automated_test_config_to_zip(zip_file, zip_dir, settings_file_path)
    ...
    test_spec_content = strip_ids_from_spec(test_specs)
    misc_settings = {}
  ...
  def strip_ids_from_spec(test_specs)
    return test_specs if test_specs['testers'].nil?
    test_specs['testers'].each do |tester_specs|
      ...
      tester_specs['test_data'].each do |test_group_specs|
        ...
        unless test_group_specs['extra_info']['criterion'].nil?
          ...
          extra_data_specs['criterion_name'] = assoc_criteria.name
        end
        test_group_specs['extra_info'] = extra_data_specs
      end
    end
    test_specs
  end
