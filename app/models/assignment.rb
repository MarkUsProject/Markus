...
class Assignment < ActiveRecord::Base
  ...
  def add_csv_groups(file)
    ...
    CSV.foreach(file) do |row|
      ...
      group = Group.where(group_name: row.first).first
      if group.nil?
        group_list << Group.new(group_name: row[0], repo_name: repo_name)
        group_search << row[0]
      end
      # Note: after_create hook build_repository might raise
      # Repository::RepositoryCollision. If it does, it adds the colliding
    ...
    groups.each do |group|
      ...
      grouping_list << Grouping.new(assignment: self, group: group, group_id: group.id)
      grouping_search << group.id
    end
    Grouping.import grouping_list
    groupings = Grouping.where(assignment_id: self.id, group_id: grouping_search) 
    groupings.each do |grouping|
      grouping.create_grouping_repository_folder
      student_list.each do |students|
