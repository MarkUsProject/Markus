    ...
    group = if new_group_name.nil?
              if members.length == 1
                ...
                if self.is_timed
                  ...
                  Group.find_or_initialize_by(group_name: student_user_name, course: self.course) do |g|
              ...
              if existing_group
                raise "Group #{new_group_name} already exists" if groupings.exists?(group_id: existing_group.id)
                existing_group
              else
                Group.new(group_name: new_group_name, course: self.course, repo_name: new_group_name)
              end
            end

    unless group.save
      raise "Failed to save group: #{group.errors.full_messages.join(', ')}"
    end

    grouping = Grouping.new(group: group, assignment: self, assessment_id: self.id)
    unless grouping.save
      group.destroy
      raise "Failed to create grouping: #{grouping.errors.full_messages.join(', ')}"
    end

    unless members.empty?
      members.each_with_index do |user_name, i|
        student = Student.joins(:user).find_by(users: { user_name: user_name })
        if student
          membership = grouping.student_memberships.build(
            role_id: student.id,
            grouping_id: grouping.id,
            membership_status: i == 0 ? StudentMembership::STATUSES[:inviter] : StudentMembership::STATUSES[:accepted]
          )
          unless membership.save
            raise "Failed to add member #{user_name} to group: #{membership.errors.full_messages.join(', ')}"
          end
        else
          raise "Student #{user_name} not found"
        end
      end
    end
    group
