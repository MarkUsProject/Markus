...
module LtiHelper
  ...
  def roster_sync(lti_deployment, role_types, can_create_users: false, can_create_roles: false)
    ...
    if member_info.blank?
      raise I18n.t('lti.no_users')
    end

    # Separate users into active and inactive based on LMS status
    lms_active_data = []
    lms_inactive_ids = []
    ...
    processed_lti_ids = []
    lms_active_data.each do |lms_user|
        ...
        Rails.logger.error I18n.t('lti.sync_failed_log',
                                  user: lms_user[:user_name],
                                  message: e.message)
        error = true
        next
    end
    ...
    ids_to_hide = (lms_inactive_ids + missing_ids).uniq
    if ids_to_hide.any?
