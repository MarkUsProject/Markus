class AutotestRunJob < ApplicationJob
  ...
  def enqueue_test_run(test_run, host_with_port, test_specs, hooks_script, ssh = nil)
    params_file = nil
    ...
    server_params = { user_type: test_run.user.type, markus_address: markus_address, server_api_key: server_api_key,
                      test_specs: test_specs, hooks_script: hooks_script, assignment_id: assignment.id,
  ...
  def perform(host_with_port, user_id, test_specs, hooks_script, test_runs)
    ssh = nil
    ...
    test_runs.each do |test_run|
      begin
        ...
        enqueue_test_run(test_run, host_with_port, test_specs, hooks_script, ssh)
      rescue StandardError => e
        unless test_run.nil?
          #TODO How to do this with specs? Keep the files too?
          error = { name: I18n.t('automated_tests.results.all_tests'),
